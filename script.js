...
function generateEventCardHTML(event) {
    ...
    const statusBadgeClass = isUpcoming ? 'upcoming' : 'past';
    const statusBadgeText = isUpcoming ? 'Upcoming' : 'Past';
    ...
    return `
        <article class="event-card-poster">
            <div class="event-poster-section" onclick="openPosterFullscreen('${event.poster || ''}', '${event.title}')">
                ${posterHTML}
                <span class="event-status-badge ${statusBadgeClass}">${statusBadgeText}</span>
            </div>
...
function renderHomepageEvents() {
    ...
    // Single event center fix - ye add karo
